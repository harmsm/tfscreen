...
def run_map(some_model,
    ...
    optimal_params : np.ndarray
        The array of estimated parameters that minimize the negative
        log-posterior.
    ...
    if result.success:
        ...
        if np.all(np.isfinite(hess_inv)):
            ...
            print("Warning: Optimization succeeded, but the estimated "
                  "covariance matrix contains non-finite values. This may "
                  "indicate an ill-conditioned problem or non-identifiable "
                  "parameters. Returning None for covariance.")
    else:
        print(f"Warning: Optimization failed. Status: {result.status}, "
              f"Message: {result.message}")
        optimal_params = None # Indicate failure
