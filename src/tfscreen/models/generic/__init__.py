...
Note
----
Most of these models require nonlinear regression to estimate parameter values.
The guess functions for these models return a good starting point guess for such
a fit. A subset of the models (flat, linear, poly_2nd, etc.) are polynomials 
that can be solved by weighted least squares (linear). For consistency, these
models still define a guess function, but that guess function returns a design
matrix 'X' rather than a 1D set of parameter guesses. Any function using this
library can run the guess function and check the dimensions of the output. If
it's 1D, the calling function should solve the problem with nonlinear least 
squares (or the like). If it's 2D, the calling function should solve the problem
