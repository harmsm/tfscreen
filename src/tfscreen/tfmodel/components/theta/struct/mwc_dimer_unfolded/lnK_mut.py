...
values K_h_l, K_h_o, K_l_o, K_h_e, K_l_e).
"""
...
from tfscreen.tfmodel.components.theta.struct.mwc_dimer_unfolded.thermo import (
    ThetaParam,
    _compute_theta,
    _population_moments,
    run_model,
...
class ModelPriors:
    """Hyperparameters for the additive ln-K MWC-dimer-unfolded theta model."""
...
def define_model(name: str,
    ...
    """
    Define the additive ln-K MWC-dimer-unfolded hierarchical model.
    ...
    and is applied only to folded-state K values (K_h_l, K_h_o, K_l_o, K_h_e,
    K_l_e).  K_u carries no epistasis term.
    ...
    # K_h_l, K_h_o, K_l_o, K_u: scalar (no effector-type dimension)
    # ------------------------------------------------------------------
    ...
    # Scalar K_h_l, K_h_o, K_l_o, K_u: shape (num_mutation,)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Optional epistasis — regularised horseshoe prior (folded-state only)
    # ------------------------------------------------------------------
    if has_epi:
    ...
    d_ln_K_u   = d_K_u_off   * sigma_d_K_u              # (M,)
    ...
    ln_K_u = ln_K_u_wt + mut_scatter(d_ln_K_u)         # (G,) — no epistasis
...
def guide(name: str,
          data: Union[GrowthData, BindingData],
          priors: ModelPriors) -> ThetaParam:
    """Variational guide for the additive ln-K MWC-dimer-unfolded model."""
    ...
    ln_K_u = ln_K_u_wt + mut_scatter(d_K_u_off * sigma_d_K_u)   # (G,) — no epistasis
...
def predict_unmeasured(
    ...
    *,
    ...
    """
    Predict theta for unmeasured genotypes using additive ln-K MWC-dimer-unfolded assembly.
...
def get_extract_specs(ctx):
    ...
    # Scalar per-genotype K values (includes K_u)
    ...
    # Per-mutation scalar Δln_K values (includes d_ln_K_u)
    mut_df = pd.DataFrame({"mutation": ctx.mut_labels, "map_mut": range(num_mut)})
    specs.append(dict(
        input_df=mut_df,
        params_to_get=["d_ln_K_h_l", "d_ln_K_h_o", "d_ln_K_l_o", "d_ln_K_u"],
        map_column="map_mut",
