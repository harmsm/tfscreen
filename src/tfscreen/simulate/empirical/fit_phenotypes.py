"""
Stage 1 of the empirical-phenotype pipeline: per-genotype MLE fits of the
growth model against *real* experimental data.

Given a processed ``ln_cfu`` DataFrame (the output of ``tfs-process-counts``)
and a set of *frozen* per-condition growth-calibration parameters (``k``, ``m``
from ``tfs-prefit-calibration``), this module fits an independent, lightly
regularized nonlinear least-squares model to each genotype's ~O(100)
observations:
...
The purpose is *not* genotype-specific truth — the per-genotype fits are
confounded (see the module notes) and, on bulk data, congression-attenuated.
The purpose is to harvest a realistic *joint distribution* of per-genotype
phenotype parameters that a later stage resamples to seed a simulation.  For
that, each fit returns both a point estimate **and** its covariance, both in
the transformed coordinate system in which the downstream distribution is
estimated and sampled.
...
def fit_phenotypes(growth_df,
    ...
    if not rows:
        warnings.warn("fit_phenotypes produced no fits (empty/filtered input).")
