"""
Stage 1.5 of the empirical-phenotype pipeline: de-attenuate the per-genotype
theta curves for congression (co-transformation).
...
from tfscreen.simulate.empirical.fit_phenotypes import (
    _hill_theta, _POWER_CLIP, _LOGIT_BOUND,
)
...
def deattenuate_congression(fits, growth_df, lam, spiked=None,
    ...
    for titrant_name, sub in growth_df.groupby("titrant_name", observed=True):
        ...
        theta_obs = np.stack([_theta_from_fit(fits[k], concs) for k in keys],
                             axis=1)
        theta_true, _ = correct_theta_matrix(
