...
def get_init_values(df, model, data, priors):
    ...
    # CORRECTED METHOD:
    # 1. Create a dummy SVI object to discover parameter shapes.
    #    We need a dummy optimizer and loss, which are not used beyond this step.
    optimizer = Adam(step_size=0.01)
    guide = AutoNormal(model)
    svi_for_init = SVI(model, guide, optimizer, loss=Trace_ELBO())

    # 2. Run svi.init() to get the initial, randomly-initialized parameters.
    ...
    # --- 1. Smart Initializations (from your data) ---

    # Guess ln_cfu0 from the mean of the earliest time points for each block
    idx = df.groupby(['ln_cfu0_block'],observed=False)['t_sel'].idxmin()
    init_df = df.loc[idx]
    init_ln_cfu0_locs = jnp.zeros(data.num_ln_cfu0_block)
    block_map = {name: i for i, name in enumerate(df['ln_cfu0_block'].cat.categories)}
    
    for _, row in init_df.iterrows():
        block_idx = block_map[row['ln_cfu0_block']]
        init_ln_cfu0_locs = init_ln_cfu0_locs.at[block_idx].set(row['ln_cfu'])
        
    init_values["ln_cfu0_hyper_locs_auto_loc"] = init_ln_cfu0_locs
    
    # Use pre-estimated values for wild-type theta
    init_values["theta_wt_auto_loc"] = priors.wt_theta_loc

    # --- 2. Default Initializations (for all other parameters) ---

    for name, proto_value in proto_params.items():
        # Skip parameters we've already set with smart guesses
