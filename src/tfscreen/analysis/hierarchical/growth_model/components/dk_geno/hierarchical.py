...
def define_model(name: str,
    ...
    ----------
    name : str
        The prefix for all Pyro sample sites (e.g., "dk_geno").
    data : GrowthData
    ...
    dk_geno_lognormal_values = torch.clamp(torch.exp(dk_geno_hyper_loc + dk_geno_offset * dk_geno_hyper_scale), max=1e30)
...
def guide(name: str,
    ...
    h_loc_loc = pyro.param(f"{name}_hyper_loc_loc", torch.tensor(priors.dk_geno_hyper_loc_loc))
    h_loc_scale = pyro.param(f"{name}_hyper_loc_scale", torch.tensor(priors.dk_geno_hyper_loc_scale),
    ...
    shift_loc = pyro.param(f"{name}_shift_loc", torch.tensor(priors.dk_geno_hyper_shift_loc))
    shift_scale = pyro.param(f"{name}_shift_scale", torch.tensor(priors.dk_geno_hyper_shift_scale),
                             constraint=torch.distributions.constraints.positive)
...
def get_guesses(name, data):
    ...
    guesses[f"{name}_offset"] = -0.8240460108562919 * torch.ones(data.num_genotype)
