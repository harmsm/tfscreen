...
- **Shape** (which model): selection is controlled by `select_by` in `cat_fit.py`. **Default `"aicc"`**: `best_model` = lowest-AICc model (small-sample-corrected AIC on the **weighted** residuals `chi2 = sum(((y-yfit)/y_std)**2)`, `aic = 2k + chi2`; `aicc=inf` when `n-k-1 <= 0`, params still reported). This is the robust default — the weighted χ² correctly weights the few informative points, which the sign-based runs test does **not**. Opt-in `select_by="adequacy"` (`select_by_adequacy`) is an **escalate-only** refinement: keep the AICc pick unless its residuals are systematically clustered (one-sided lower-tail Wald-Wolfowitz runs test, `runs_p < adequacy_alpha`), then move to the lowest-AICc adequate model that is **no simpler** (`k >=` the AICc pick's `k`). It **never demotes**, so it cannot collapse a confident curved fit to `flat` — the failure mode of the earlier (removed) "simplest-adequate" rule, which on noisy heteroscedastic (logit) data let the diluted runs test override AICc and demote real curves to `flat` (killing the omnibus "real" call). Per-model diagnostics are reported but **do not gate the default**: `runs_p|*` (sign-based, robust to `y_std` scale; needs `n >= _MIN_RUNS_N` (4), real power only at `n >~ 8`) and weighted-χ² `gof_p|*` (`goodness_of_fit_p`, depends on `y_std` scale). `shape` = qualitative form of `best_model` (`flat`/`linear`/`sigmoid`/`peaked`/`biphasic`, via `_SHAPE_BY_MODEL`); `shape_status` = diagnostic on the **selected** model's residuals (`adequate` / `misfit` / `unassessable` / `none`, via `_shape_status`); `aicc_best_model` records the AICc pick (differs from `best_model` only in adequacy mode when it escalates). This is a form axis, **orthogonal to** the magnitude/`response_class` axis below.
