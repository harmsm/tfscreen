...
- **Shape** (which model): **adequacy-first** selection in `cat_fit.py` (`select_by_adequacy`), *not* pure AICc. Each model's AICc is still computed (small-sample-corrected, on the **weighted** residuals `chi2 = sum(((y-yfit)/y_std)**2)`, `aic = 2k + chi2`; `aicc=inf` when `n-k-1 <= 0`, params still reported) and reported per model, but the selected `best_model` is the **simplest model whose residuals are not systematically clustered** — judged by a **one-sided (lower-tail) Wald-Wolfowitz runs test** on the sign sequence of the residuals ordered by x (`runs_p >= adequacy_alpha`, default 0.05) — with AICc breaking ties *within* a complexity (`k`) tier. This fixes AICc's small-sample tendency to crown a simple model (e.g. `linear`) that clearly misfits: at ~8 pts the parsimony penalty `2k(k+1)/(n-k-1)` makes a 4-param curve need a huge χ² drop to win, so a structured-residual `linear` used to slip through. The runs test is **sign-based → robust to `y_std` miscalibration** (important on the logit scale); it needs `n >= _MIN_RUNS_N` (4) usable residuals and has real power only at `n >~ 8`. `shape` = the qualitative form of `best_model` (`flat`/`linear`/`sigmoid`/`peaked`/`biphasic`, via `_SHAPE_BY_MODEL`); `shape_status` = `adequate` / `unmodeled` (all fits fail the runs test → fall back to AICc-best) / `unassessable` (no computable runs test) / `none`. `aicc_best_model` records what AICc alone would have picked (for transparency when they diverge). A weighted-χ² **goodness-of-fit** p (`goodness_of_fit_p` → `gof_p|*`, `best_model_gof_p`) is also reported but **does not gate** selection (it depends on the `y_std` scale, unlike the runs test). This selection is a form axis, **orthogonal to** the magnitude/`response_class` axis below.
- **Magnitude** (distinguishable from zero): a post-hoc pass, `cat_assess.py`, grading the *best* model against zero. Per-point `sig_nonzero` z-test at the observed x; a per-curve **omnibus** Wald/χ² `W = yhat @ pinv(Σ_yhat) @ yhat` (df = rank(Σ) = n_params) using the **full** prediction covariance (`predict_with_error(..., full_cov=True)` = `J·Cov·Jᵀ`), which rewards both magnitude and how-many-points and handles the strong correlation among predicted points; then **Benjamini-Hochberg** across curves → `omnibus_q`.

`equiv_zero` (a point's whole CI ⊂ `[-delta, delta]`) is the confidence axis that separates "confidently flat" from "too noisy to tell". `delta` defaults to `delta_c * median(predicted y_std)` (a **detectability** threshold, computed globally in `cat_response.py` after all fits — hence the post-hoc pass); pass `--delta` for a fixed biological region. `response_class` is 3-valued with **equivalence taking precedence over significance** (a tiny-but-significant offset inside the ROPE is still practically zero): `confident_zero` (all points equiv) → else `real` (omnibus_q < alpha) → else `indeterminate` (the "flat because error is high" bucket to exclude downstream).

Outputs: rollups (`best_model`/`aicc_best_model`/`shape`/`shape_status`/`best_model_runs_p`/`best_model_gof_p`, `omnibus_p/q`, `n_nonzero`, `all_equiv_zero`, `response_class`) land in `{prefix}.csv`; `{prefix}_assessment.csv` is the self-contained per-point record — `model` (best model name), `x`, observed `y_obs`/`y_std`, fitted `y_model`/`y_model_std` (curve value + propagated fit error, **not** the observed error), then `z`/`sig_nonzero`/`direction`/`equiv_zero`; `{prefix}_predictions.csv` holds only each group's **best** model (columns `model,x,y_model,y_model_std,is_best_model`; `best_only=True` threaded `cat_response→cat_fit` so the all-model curve is never built) unless `--write_all_predictions`. Note `y_model`/`y_model_std` are the **model prediction** at each observed x, distinct from `y_obs`/`y_std` (the experimental point + its input error). `cat_fit` returns a 3-tuple `(flat_output, pred_df, assess_df)`; `cat_response` a 4-tuple `(results_df, predictions_df, assessment_df, delta)`.
