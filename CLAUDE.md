...
tfs-fit-genotypes          # Per-genotype MLE fits of the growth model (+ optional congression de-attenuation; spiked genotypes come from --library_config)
...
- **`generative/components/`** — Pluggable model components selected via the YAML config's `components:` section (registry key noted in parens where it differs from the directory name):
  ...
  - `transformation/`: `single`, `mixture` (`empirical`/`logit_norm` were removed; `ModelOrchestrator` refuses them by name with a message, `_RETIRED_TRANSFORMATIONS`). `_congression.update_thetas` (the old E[max] θ operator) is kept only for Stage 1.5 of `tfs-fit-genotypes`
