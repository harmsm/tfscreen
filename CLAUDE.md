...
- **`generative/model.py`** — Numpyro probabilistic model definition. The generative model is:
  ...
  With `transformation: mixture` each genotype's cells are split into classes (clean, plus K congressed classes built from fixed co-resident sets) with cell-level θ/A/dk_geno; every class is grown through the same pipeline on a leading class axis and the classes are mixed as `ln_cfu = ln_cfu0 + logsumexp_c(log w_c + G_c)`. The transformation component's `cell_classes(focal, population, params, data)` builds the classes (`single`: one class); `NEEDS_POPULATION` makes `jax_model` supply library-ordered θ/activity/dk_geno (`return_population=True`, or `GrowthData.external_*_population` in prediction).
...
| `simulate/cell_rules.py` | Pluggable rules combining a co-transformed cell's plasmids: `THETA_RULES` (`max`: highest-θ plasmid sets θ and activity) and `DK_RULES` (`dilution`: share-weighted dk_geno), chosen by `congression_theta_rule`/`congression_dk_rule`. Growth is then `thermo_to_growth.growth_rate_one_condition`, the same formula used per genotype. Never combine whole growth rates across plasmids |
...
**Required practice**: maintain **one** `run_config.yaml` per experiment and pass that same file to `tfs-simulate`, `tfs-process-fastq` and `tfs-configure-model --library_config`. Nothing in the pipeline checks that the three saw the same file: the genotype-set check in `tfs-configure-model` catches a genetics mismatch, but `library_mixture` (which only `tfs-configure-model` and `tfs-simulate` read) can silently drift. Use `examples/process_raw/library_config.yaml` only when you need a minimal standalone library config (e.g. for processing real data without a matching simulation); note it carries `library_mixture` as well as the genetics keys.
