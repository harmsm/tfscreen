...
This file was started on 2026-09-10, at package version 0.4.3. Entries fall
into two kinds:
...
- **MAP / AutoDelta scrambled per-genotype latents across genotypes.**
  Several components sampled per-genotype latents in a plate sized
  `data.batch_size` instead of the library size. Any numpyro autoguide sizes
  its parameters from a traced batch, so `AutoDelta` held one value per batch
  *position*, and every step wrote whichever genotypes were in the batch onto
  those positions. This also happened at full batch: the full-batch training
  index lists binding genotypes first and reshuffles the rest on every step.
  Binding genotypes, which always occupy the same leading slots, were the only
  ones fit consistently.
  - Affected paths: `tfs-fit-model --analysis_method map`, Laplace posteriors
    from MAP checkpoints (`tfs-sample-posterior`), the pre-MAP that precedes
    SVI, and `tfs-prefit-calibration`. Prefit derives its per-condition
    `condition_growth` `k`/`m` prior pins from this MAP, so those pins were
    fit alongside scrambled nuisance latents.
  - Not affected: SVI with the component guide (the default). Component
    guides hold library-sized variational parameters and slice them with
    `batch_idx`, so this bug does not explain SVI miscalibration.
  - Affected sites: `dk_geno_offset` (`dk_geno/hierarchical_geno`),
    `activity_offset` and `activity_local_scale` (`activity/hierarchical_geno`,
    `activity/horseshoe_geno`), `ln_cfu0_offset` (`ln_cfu0/hierarchical`),
    `ln_cfu0_offset_geno` (`ln_cfu0/hierarchical_factored`),
    `theta_logit_theta_offset` (`theta/categorical_geno`, joint and
    binding-only modes), and `theta_growth_noise_epsilon`
    (`noise/logit_normal`).
  - Fix: each of these latents is now sampled in a library-sized plate,
    `pyro.plate(f"{name}_genotype_plate", data.num_genotype)`, then sliced with
    `[..., data.batch_idx]` whenever the value is library-sized. A smaller
    value is an already-sliced substitution from the posterior forward pass
    and is used as-is. This is the pattern `theta/hill_geno` already followed.
  - **Action:** MAP, Laplace and prefit-calibration outputs produced before
    this fix should be regenerated, including the prefit-pinned per-condition
    growth priors.
- `get_map_posteriors` silently clipped genotype indices to the latent's
  length, so every genotype past `batch_size` got the last batch position's
...
- **SVI guide selection.** `tfs-fit-model --guide_type` chooses the
  variational family for `--analysis_method svi`:
  - `component` (default): the guide assembled from the model components.
  - numpyro autoguides: `delta`, `auto_normal`, `auto_diagonal_normal`,
    `auto_multivariate_normal`, `auto_low_rank_multivariate_normal`. numpyro
    class names (e.g. `AutoNormal`) are accepted, case-insensitively.
  - `--guide_rank` (low-rank guide) and `--guide_init_scale` (every autoguide
    except `delta`) are validated before any fitting starts. They, and a
    non-default `--guide_type`, are rejected for `map` and `nuts`.
  - An autoguide's location starts at the pre-MAP point (`init_to_value`) when
    `--pre_map_num_epoch > 0`, otherwise at the configured guesses.
  - Because per-genotype latents are now library-sized, every autoguide,
    including the dense `auto_multivariate_normal`, is valid under genotype
    mini-batching. The dense guide's cost is O(D²) memory; its size is printed
    when fitting starts, with a warning above 4 GB.
- `RunInference.setup_svi(guide_type, guide_kwargs, init_values)`, backed by
  module-level `AUTOGUIDES`, `GUIDE_TYPES`, `resolve_guide_type` and
  `check_guide_kwargs` in `tfmodel/inference/run_inference.py`.
- Checkpoints record `guide_type` and `guide_kwargs`.
  `restore_svi_from_checkpoint` rebuilds the recorded guide; checkpoints
  without the metadata are treated as `component`.
- `tfmodel/inference/batch_safety.py`:
  - `find_batch_dependent_latents` traces a model at two batch sizes (under
    `jax.eval_shape`, so it costs nothing even on a full library) and reports
    latent sites whose shape follows the batch.
  - Also `find_orchestrator_batch_dependent_latents` and
    `orchestrator_latent_dimension`.
- `tfs-simulate`: `base_growth_data` gains `choose_by: random` plus `num`, as
  an alternative to listing `genotypes`. It draws wt plus `num` genotypes from
  those that survived with growth data, excluding the spiked and in-library
  (`binding_data.library_binding`) binding genotypes, so base-growth anchors
  are disjoint from binding anchors. It fails fast if combined with
  `genotypes`/`rates`, if `num` is missing, or if too few genotypes are
...
- Tests:
  - `tests/tfscreen/tfmodel/inference/test_batch_safety.py`: registry-wide
    batch-safety check for every per-genotype component, in joint and
    binding-only modes, plus a permuted-batch ordering test.
  - `tests/tfscreen/tfmodel/inference/test_guide_selection.py`: autoguide
    refusal, plus fit → checkpoint → restore → posterior round trips for each
    autoguide.
  - Guide-selection coverage in the `fit_model`, `sample_posterior` and
    `RunInference` tests, and an end-to-end
    `test_configure_run_autoguide_smoke`.
- `CLAUDE.md` documents the library-size-plate rule for new components and
  the guide-selection flow.

### Changed

- `RunInference.run_optimization` refuses to fit with any numpyro autoguide
  (AutoDelta included) when the model has batch-dependent latents. The only
  remaining case is `theta_growth_noise=beta`, whose `{name}_dist` latent is
  the noisy θ itself and cannot be library-sized.
  - **Behavior change:** `--analysis_method map` and `tfs-prefit-calibration`
    now raise for `theta_growth_noise=beta` instead of silently producing
    scrambled values. The component guide still fits it.
- Per-genotype latent priors and guides are no longer wrapped in
  `scale_vector`. They are evaluated exactly over the whole library every
  step, and only the likelihood carries the mini-batch scale. The objective
  is unchanged in expectation. Per-step cost for these sites grows from
  O(batch) to O(library); at production sizes (~200k genotypes, 65,536 per
  batch) that is at worst about 3× for those sites.
- Per-genotype latent plates are renamed from `shared_genotype_plate` /
  `theta_genotype_plate` to `{name}_genotype_plate`. The observers keep
  `shared_genotype_plate`. Code that matches plates on the substring
  `genotype` (`_get_genotype_dim_map`, `prediction.py`, `model_stats`) is
  unaffected.
- `tfs-sample-posterior` routes by the checkpoint's recorded `guide_type`:
  `delta` goes to the Laplace approximation, every other guide to sampling
  from the guide. Checkpoints without the metadata keep the old
  `*_auto_loc` name heuristic, which on its own would misroute AutoNormal
  checkpoints to Laplace.
- `tfs-fit-model` refuses to resume an SVI checkpoint under a different
  `--guide_type` than the one that wrote it.
- `examples/simulate-and-analyze` now matches the current production fit
  (`dev/run.sh`):
  - **Simulation:** congression at λ = 0.3572
    (`transformation_poisson_lambda`), 20 stratified in-library binding
    genotypes (`binding_data.library_binding`), and 20 random base-growth
    genotypes disjoint from the binding sets (`base_growth_data`,
    `k_ref` 0.015366, noise 0.0005 hr⁻¹).
  - **Fit** (`run.sh`): `--base_growth_df`, `--transformation_model
    empirical --transformation_lambda 0.3572 0.1296`, `--batch_size 65536`,
    and the triple mutant M42I/H74A/K84L added to `--spiked` (it is in the
    simulated `spiked_seqs`). Prefit runs without `--pin_m`.
  - **Downstream steps** (`run.sh`): fit convergence tolerance 5e-7, 500
    posterior samples, a new `tfs-predict-epistasis --scale_constant -0.6159`
    step, and growth predicted on a fixed-seed genotype subset.
- `get_posteriors` drops autoguide auxiliary sites (e.g. `AutoContinuous`'s
  flattened `_auto_latent`, shape samples × D) instead of writing them to the
  posterior `.h5`.

### Removed

- The `block_diagonal` value of `setup_svi(guide_type=...)`. It built a
  full-rank `AutoMultivariateNormal` rather than a block-diagonal guide, and
  is superseded by `auto_multivariate_normal`. The name is reserved for a
  true per-genotype block guide.
- `RunInference.get_pathfinder_posteriors`, an exploratory Blackjax
  Pathfinder sampler. It was numerically unstable and depended on `blackjax`,
  which is not a declared dependency.
- `scripts/run_block_svi_standalone.py`, superseded by
  `tfs-fit-model --guide_type`.

### Known issues

- On the 9-genotype smoke dataset, MAP started from scratch and run with
  mini-batches reached a much worse full-data optimum than full-batch MAP
  (potential energy −1551 vs −3164; lower is better). Started from the
  full-batch optimum, mini-batch MAP stayed there, so the objectives agree and
  the gap is an optimization problem. Not yet checked at production scale.
- The pre-MAP that precedes component-guide SVI does not warm-start it:
  AutoDelta's `{site}_auto_loc` parameter names match none of the component
  guide's parameters, so the hand-off is ignored. This predates the changes
  above, which leave it as is. Autoguides are warm-started correctly.

### Credits

- Suryansh (GitHub: [alphabreacher](https://github.com/alphabreacher)) ran
  the pilot calibration study that found `AutoMultivariateNormal` markedly
  better calibrated than the component guides on small simulated datasets.
  That result motivated the guide-selection work above, and his original
  guide and runner commit is preserved in the history.
