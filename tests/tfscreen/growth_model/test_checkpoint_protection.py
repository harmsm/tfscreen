...
def mock_growth_model(mocker):
    mock_gm_class = mocker.patch("tfscreen.growth_model.scripts.fit_model_cli.read_configuration")
...
def test_analyze_theta_checkpoint_exists_with_resume(mock_growth_model, mock_run_inference):
    ...
    with patch("os.path.exists", return_value=True):
        with patch("tfscreen.growth_model.scripts.fit_model_cli._run_svi") as mock_run_svi:
            fit_model(
                config_file="config.yaml",
                seed=1,
                out_prefix="test_root",
