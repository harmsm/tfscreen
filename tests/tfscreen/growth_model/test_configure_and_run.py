...
from tfscreen.growth_model.configuration_io import (
    read_configuration,
    _update_dataclass
)
...
@pytest.fixture
def mock_gm(mocker):
    mock_gm_class = mocker.patch("tfscreen.growth_model.scripts.configure_model_cli.GrowthModel")
...
def test_read_configuration_logic(tmpdir, mocker):
    ...
    mock_gm_class = mocker.patch("tfscreen.growth_model.configuration_io.GrowthModel")
    mock_gm_inst = mock_gm_class.return_value
    mock_gm_inst.init_params = {"param1": 0.0, "param2": jnp.zeros((1,)), "param3": 0.0}
    
    # Mock update_dataclass
    mocker.patch("tfscreen.growth_model.configuration_io._update_dataclass"
...
def test_read_configuration_errors(tmpdir):
    ...
    with patch("tfscreen.growth_model.configuration_io.GrowthModel"):
        ...
        with patch("tfscreen.growth_model.configuration_io.GrowthModel") as mock_gm_class:
            mock_gm_inst = mock_gm_class.return_value
...
def test_mains(mocker):
    ...
    mock_run = mocker.patch("tfscreen.growth_model.scripts.configure_model_cli.configure_model", autospec=True)
    from tfscreen.growth_model.scripts.configure_model_cli import main
    main()
    assert mock_run.called

    mocker.patch("sys.argv", ["tfs-run", "c.yaml", "--seed", "42"])
    mock_run_analysis = mocker.patch("tfscreen.growth_model.scripts.fit_model_cli.fit_model", autospec=True)
    from tfscreen.growth_model.scripts.fit_model_cli import main as main_run
    main_run()
...
def test_fit_model_svi_full(tmpdir, mocker):
    with patch("os.path.exists", return_value=False):
        ...
        mocker.patch("tfscreen.growth_model.scripts.fit_model_cli.read_configuration", return_value=(mock_gm_run, {"p":1.0}))

        # Mock write_configuration to avoid yaml problems with mocks
        mock_ri_class = mocker.patch("tfscreen.growth_model.scripts.fit_model_cli.RunInference")
        mock_svi = mocker.patch("tfscreen.growth_model.scripts.fit_model_cli._run_svi")
        mock_map = mocker.patch("tfscreen.growth_model.scripts.fit_model_cli._run_map")
