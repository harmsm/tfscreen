"""Tests for prior_predictive_cli.py — draws and writes synthetic prior datasets."""
import os
import pytest
import numpy as np
import h5py
...
class TestWriteH5:
    ...
    def test_creates_file(self, tmp_path):
        from tfscreen.tfmodel.scripts.prior_predictive_cli import _write_h5
...
class TestSamplePrior:
    ...
    def test_writes_csv_and_h5_for_one_dataset(self, tmp_path):
        from tfscreen.tfmodel.scripts.prior_predictive_cli import sample_prior

        fake_gm = MagicMock()
        fake_growth_df = pd.DataFrame({"ln_cfu": [1.0, 2.0]})
        latent_params = {"activity": np.array([[1.0]])}

        with patch("tfscreen.tfmodel.scripts.prior_predictive_cli.read_configuration",
                   return_value=(fake_gm, {})), \
             patch("tfscreen.tfmodel.scripts.prior_predictive_cli.draw_prior",
                   return_value=({}, latent_params)), \
             patch("tfscreen.tfmodel.scripts.prior_predictive_cli.growth_df_from_prior",
