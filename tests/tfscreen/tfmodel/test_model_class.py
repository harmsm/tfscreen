...
from tfscreen.tfmodel.model_class import (
    ModelClass,
    _read_growth_df,
    _read_binding_df,
    _build_growth_tm,
...
def test_read_growth_df(mocker, base_growth_df):
    ...
    mocker.patch("tfscreen.tfmodel.model_class.add_group_columns", return_value=base_growth_df)
...
def test_read_growth_df_no_replicate(mocker, base_growth_df):
    ...
    mocker.patch("tfscreen.util.dataframe.check_columns")
    mocker.patch("tfscreen.tfmodel.model_class.add_group_columns", return_value=df_no_rep)
...
def test_read_binding_df(mocker):
    ...
    mocker.patch("tfscreen.util.dataframe.check_columns")
    mocker.patch(
        "tfscreen.tfmodel.model_class.add_group_columns",
...
def test_model_class_invalid_component(mocker):
    ...
    mocker.patch("tfscreen.tfmodel.model_class._read_growth_df")
    mocker.patch("tfscreen.tfmodel.model_class._build_growth_tm", return_value=mock_growth_tm)
    mocker.patch("tfscreen.tfmodel.model_class._read_binding_df")
    mocker.patch("tfscreen.tfmodel.model_class._build_binding_tm", return_value=mock_binding_tm)
    mocker.patch("tfscreen.tfmodel.model_class.populate_dataclass", return_value=MagicMock())
    mocker.patch("tfscreen.tfmodel.model_class._setup_batching", return_value={"batch_idx": jnp.array([0]), "batch_size": 1, "scale_vector": jnp.array([1.0]), "num_binding": 0})
    ...
    with patch("tfscreen.tfmodel.model_class.model_registry", {}):
...
def test_initialize_data_spiked(mocker):
    ...
    mocker.patch("tfscreen.tfmodel.model_class._build_binding_tm", return_value=mock_binding_tm)
    mocker.patch("tfscreen.tfmodel.model_class.populate_dataclass", return_value=MagicMock())
    mocker.patch("tfscreen.tfmodel.model_class._setup_batching", return_value={"batch_idx": jnp.array([0,1]), "batch_size": 2, "scale_vector": jnp.array([1.0, 1.0]), "num_binding": 0})
    mocker.patch("tfscreen.tfmodel.model_class.ModelClass._initialize_classes")
...
def test_initialize_classes_logic(mocker):
    ...
    with patch.dict("tfscreen.tfmodel.model_class.model_registry", {
        ...
        for k in ["condition_growth", "growth_transition", "ln_cfu0", "dk_geno", "activity", "theta", "transformation", "theta_growth_noise", "theta_binding_noise", "growth_noise"]:
            for sub_k in tfscreen.tfmodel.model_class.model_registry[k]:
                tfscreen.tfmodel.model_class.model_registry[k][sub_k].get_priors.return_value = {}
                tfscreen.tfmodel.model_class.model_registry[k][sub_k].get_guesses.return_value = {}
...
_BINDING_WEIGHT_PATCHES = [
    "tfscreen.tfmodel.model_class._read_growth_df",
    "tfscreen.tfmodel.model_class._read_binding_df",
]
...
def _make_model_for_binding_weight(mocker, n_growth, n_binding, binding_weight=None):
    ...
    for path in _BINDING_WEIGHT_PATCHES:
        mocker.patch(path)
    mocker.patch(
        "tfscreen.tfmodel.model_class._build_growth_tm",
        return_value=mock_growth_tm,
    )
    mocker.patch(
        "tfscreen.tfmodel.model_class._build_binding_tm",
        return_value=mock_binding_tm,
    )
    mocker.patch(
        "tfscreen.tfmodel.model_class._setup_batching",
        return_value=_SETUP_BATCHING_RETURN.copy(),
    )
    mocker.patch(
        "tfscreen.tfmodel.model_class.populate_dataclass",
        return_value=MagicMock(),
    )
    mocker.patch(
        "tfscreen.tfmodel.model_class.ModelClass._initialize_classes"
    )
