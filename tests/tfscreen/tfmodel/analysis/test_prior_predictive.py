...
def test_draw_prior_returns_dicts(dummy_orchestrator):
    predictions, latent_params = draw_prior(dummy_orchestrator, rng_key=0, num_draws=1)
    assert isinstance(predictions, dict)
    assert isinstance(latent_params, dict)


def test_draw_prior_contains_growth_pred(dummy_orchestrator):
    predictions, _ = draw_prior(dummy_orchestrator, rng_key=0, num_draws=1)
    assert "growth_pred" in predictions


def test_draw_prior_growth_pred_shape(dummy_orchestrator):
    predictions, _ = draw_prior(dummy_orchestrator, rng_key=0, num_draws=3)
...
def test_draw_prior_latent_params_shape(dummy_orchestrator):
    _, latent_params = draw_prior(dummy_orchestrator, rng_key=0, num_draws=3)
...
def test_draw_prior_reproducible(dummy_orchestrator):
    _, p1 = draw_prior(dummy_orchestrator, rng_key=42, num_draws=1)
    _, p2 = draw_prior(dummy_orchestrator, rng_key=42, num_draws=1)
    for k in p1:
        np.testing.assert_array_equal(p1[k], p2[k])


def test_draw_prior_different_seeds(dummy_orchestrator):
    _, p1 = draw_prior(dummy_orchestrator, rng_key=0, num_draws=1)
    _, p2 = draw_prior(dummy_orchestrator, rng_key=1, num_draws=1)
    # At least one parameter should differ across seeds.
    any_diff = any(not np.array_equal(p1[k], p2[k]) for k in p1 if k in p2)
    assert any_diff


def test_draw_prior_multiple_independent_draws(dummy_orchestrator):
    _, latent_params = draw_prior(dummy_orchestrator, rng_key=0, num_draws=5)
...
def test_growth_df_from_prior_returns_dataframe(dummy_orchestrator):
    _, latent_params = draw_prior(dummy_orchestrator, rng_key=0, num_draws=1)
    df = growth_df_from_prior(dummy_orchestrator, latent_params)
    assert isinstance(df, pd.DataFrame)


def test_growth_df_from_prior_same_rows(dummy_orchestrator):
    _, latent_params = draw_prior(dummy_orchestrator, rng_key=0, num_draws=1)
    df = growth_df_from_prior(dummy_orchestrator, latent_params)
    assert len(df) == len(dummy_orchestrator.growth_df)


def test_growth_df_from_prior_has_ln_cfu(dummy_orchestrator):
    _, latent_params = draw_prior(dummy_orchestrator, rng_key=0, num_draws=1)
    df = growth_df_from_prior(dummy_orchestrator, latent_params)
...
def test_growth_df_from_prior_preserves_metadata(dummy_orchestrator):
    _, latent_params = draw_prior(dummy_orchestrator, rng_key=0, num_draws=1)
    df = growth_df_from_prior(dummy_orchestrator, latent_params)
...
def test_growth_df_from_prior_noise_changes_ln_cfu(dummy_orchestrator):
    _, latent_params = draw_prior(dummy_orchestrator, rng_key=0, num_draws=1)
    df_clean = growth_df_from_prior(dummy_orchestrator, latent_params, noise_rng=None)
    rng = np.random.default_rng(7)
    df_noisy = growth_df_from_prior(dummy_orchestrator, latent_params, noise_rng=rng)
...
def test_growth_df_from_prior_different_draws(dummy_orchestrator):
    _, latent_params = draw_prior(dummy_orchestrator, rng_key=0, num_draws=5)
    df0 = growth_df_from_prior(dummy_orchestrator, latent_params, draw_idx=0)
    df1 = growth_df_from_prior(dummy_orchestrator, latent_params, draw_idx=1)
    # Different draws should generally produce different predictions.
    assert not np.allclose(df0["ln_cfu"].values, df1["ln_cfu"].values)


def test_latent_params_compatible_with_predict(dummy_orchestrator):
