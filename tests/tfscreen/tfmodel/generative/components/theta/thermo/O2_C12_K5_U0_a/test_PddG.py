...
from tfscreen.tfmodel.generative.components.theta.thermo.O2_C12_K5_U0_a.PddG import (
    STRUCTURE_NAMES,
    ModelPriors,
    _get_struct_perm,
    _project_ddG,
...
def test_registry_entry():
    ...
    assert "thermo.O2_C12_K5_U0_a.PddG" in model_registry["theta"]
    import tfscreen.tfmodel.generative.components.theta.thermo.O2_C12_K5_U0_a.PddG as mod
    assert model_registry["theta"]["thermo.O2_C12_K5_U0_a.PddG"] is mod
...
def _make_ddG_posteriors(T, M, P=0, S=5, seed=0):
    """Fake posteriors for mwc_dimer lnK_ddG_prior.

    KEY DIFFERENCE from lnK_mut: d_ln_K_h_e and d_ln_K_l_e are (S, M)
    with no T dimension; epi_h_e and epi_l_e are (S, P) not (S, T, P).
    """
...
class TestDdGPriorPredictUnmeasured:
    ...
    def test_multiple_titrants_broadcast_correctly(self):
        ...
        T, M, S = 2, 2, 8
        post = _make_ddG_posteriors(T, M, S=S)
        kw = dict(
    ...
    def test_scalar_d_he_le_matches_lnK_mut_when_replicated(self):
        """ddG_prior with scalar d_h_e should equal lnK_mut when that scalar is replicated T times."""
        T, M, S = 2, 2, 5
        ...
        post_mut = {}
        post_mut["theta_ln_K_h_l_wt"] = post_ddG["theta_ln_K_h_l_wt"].copy()
        post_mut["theta_ln_K_h_o_wt"] = post_ddG["theta_ln_K_h_o_wt"].copy()
        post_mut["theta_ln_K_l_o_wt"] = post_ddG["theta_ln_K_l_o_wt"].copy()
        post_mut["theta_ln_K_h_e_wt"] = post_ddG["theta_ln_K_h_e_wt"].copy()
        post_mut["theta_ln_K_l_e_wt"] = post_ddG["theta_ln_K_l_e_wt"].copy()
        post_mut["theta_d_ln_K_h_l"]  = post_ddG["theta_d_ln_K_h_l"].copy()
        post_mut["theta_d_ln_K_h_o"]  = post_ddG["theta_d_ln_K_h_o"].copy()
        post_mut["theta_d_ln_K_l_o"]  = post_ddG["theta_d_ln_K_l_o"].copy()
