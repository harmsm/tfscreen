...
class TestRunModel:
    ...
    def test_nontrivial_batch_idx_selects_correct_genotypes(self, mock_data):
        ...
        tp_full = ThetaParam(
            ln_K_op=ln_K_op, ln_K_HL=ln_K_HL, ln_K_E=ln_K_E,
            tf_total=650.0, op_total=25.0,
            mu=jnp.zeros((T, C, 1)), sigma=jnp.ones((T, C, 1)),
        )

        # Non-trivial (reordered, non-prefix) global genotype selection,
        # exactly as produced by get_batch(): batch_idx holds the global
        # indices, geno_theta_idx is always a local arange(batch_size).
        ...
        theta_all = _compute_theta(
            tp_full.ln_K_op, tp_full.ln_K_HL, tp_full.ln_K_E,
        ...
        # Sanity check that this is NOT the same as the (buggy) prefix selection.
