...
def _sum_log_prob(model_trace):
    """Sum log-probabilities of all observed sample sites in a Pyro trace."""
    total = 0.0
...
def _make_growth_data(n_rep=1, n_time=2, n_cpre=1, n_csel=1,
    ...
    mask_val = torch.ones(shape, dtype=torch.bool) if all_good else torch.zeros(shape, dtype=torch.bool)
    return MockGD(
        ...
        batch_size=n_geno,
        ln_cfu=torch.full(shape, ln_cfu_val),
        ln_cfu_std=torch.full(shape, ln_cfu_std_val),
        good_mask=mask_val,
        scale_vector=torch.ones((n_geno,)),
    )
...
def _make_binding_data(n_tname=1, n_tconc=3, n_geno=3,
    ...
    return MockBD(
        num_titrant_name=n_tname, num_titrant_conc=n_tconc,
        batch_size=n_geno,
        theta_obs=torch.full(shape, theta_val),
        theta_std=torch.full(shape, theta_std_val),
        good_mask=mask_val,
        scale_vector=torch.ones((n_geno,)),
    )
...
class TestGrowthObserveLogLikelihood:
    ...
    def test_perfect_prediction_beats_imperfect(self):
        ...
        def run(pred):
            ...
            m = poutine.condition(observe, data={
                "growth_nu": torch.tensor(nu_val),
                "growth_growth_obs": data.ln_cfu,
            })
            t = poutine.trace(m).get_trace("growth", data, pred)
    ...
    def test_known_log_prob(self):
        ...
        pyro.clear_param_store()
        # Use poutine.do for growth_nu so it is substituted without being
        # marked as observed (poutine.condition would make it count in _sum_log_prob)
        m = poutine.do(observe, data={"growth_nu": torch.tensor(nu_val)})
        m = poutine.condition(m, data={"growth_growth_obs": data.ln_cfu})
        t = poutine.trace(m).get_trace("growth", data, pred)
        lp = _sum_log_prob(t)

        # Reference: StudentT(20, 10.0, 0.5).log_prob(10.0)
        ref_single = float(dist.StudentT(nu_val, 10.0, 0.5).log_prob(torch.tensor(10.0)))
        n_obs = 1 * 2 * 1 * 1 * 1 * 2 * 3
    ...
    def test_residual_reduces_log_prob(self):
        ...
        def run(delta):
            ...
            t = poutine.trace(m).get_trace("growth", data, data.ln_cfu + delta)
            return _sum_log_prob(t)
    ...
    def test_mask_zeroes_log_prob(self):
        ...
        def run(data):
            ...
            return poutine.trace(m).get_trace("growth", data, pred)

        t_good = run(data_good)
        t_masked = run(data_masked)

        assert "growth_growth_obs" in t_good.nodes
        assert "growth_growth_obs" in t_masked.nodes
...
class TestBindingObserveLogLikelihood:
    ...
    def test_perfect_prediction_beats_imperfect(self):
        ...
        def run(pred):
            pyro.clear_param_store()
            m = poutine.condition(observe, data={"binding_binding_obs": data.theta_obs})
            t = poutine.trace(m).get_trace("binding", data, pred)
            return _sum_log_prob(t)
    ...
    def test_known_log_prob(self):
        ...
        ref_single = float(dist.Normal(0.3, 0.05).log_prob(torch.tensor(0.3)))
        n_obs = 1 * 3 * 3
        npt.assert_allclose(lp, n_obs * ref_single, rtol=1e-5)
    ...
    def test_residual_reduces_log_prob(self):
        ...
        def run(delta):
            pyro.clear_param_store()
            m = poutine.condition(observe, data={"binding_binding_obs": data.theta_obs})
            t = poutine.trace(m).get_trace("binding", data, data.theta_obs + delta)
            return _sum_log_prob(t)
...
class TestDistributionLogProbs:
    """
    Pure distribution log-prob tests that verify numerical values match
    between NumPyro and Pyro.
    """

    def test_student_t_at_loc(self):
        """StudentT(df, loc, scale).log_prob(loc) is independent of loc."""
        lp = float(dist.StudentT(20.0, 5.0, 0.5).log_prob(torch.tensor(5.0)))
    ...
    def test_normal_at_loc(self):
        ...
        lp = float(dist.Normal(3.0, 0.05).log_prob(torch.tensor(3.0)))
        import math
    ...
    def test_half_normal_at_zero(self):
        ...
        lp = float(dist.HalfNormal(1.0).log_prob(torch.tensor(0.0)))
        import math
        expected = math.log(2) - math.log(1.0) - 0.5 * math.log(2 * math.pi)
        npt.assert_allclose(lp, expected, rtol=1e-5)

    def test_gamma_known_value(self):
        """Gamma(shape=2, rate=0.1).log_prob(20) — reference value."""
        lp = float(dist.Gamma(2.0, 0.1).log_prob(torch.tensor(20.0)))
