...
from tfscreen.analysis.hierarchical.growth_model.scripts.summarize_grid_cli import (
    _flatten_calib_stats,
...
CALIB_STATS = {
    "pct_success": 1.0, "rmse": 0.08, "normalized_rmse": 0.12,
    "pearson_r": 0.93, "spearman_r": 0.92, "r_squared": 0.86,
    "mean_error": 0.002, "coverage_prob": 0.94,
    "residual_corr": 0.01, "residual_corr_p_value": 0.5,
    "bp_p_value": 0.4, "n_params": 12, "n_obs": 300,
}


class TestFlattenCalibStats:

    def test_all_keys_prefixed(self):
        flat = _flatten_calib_stats(CALIB_STATS)
        for key in CALIB_STATS:
            assert f"calib_{key}" in flat

    def test_values_preserved(self):
        flat = _flatten_calib_stats(CALIB_STATS)
        assert flat["calib_rmse"] == pytest.approx(0.08)
        assert flat["calib_n_params"] == 12
        assert flat["calib_pearson_r"] == pytest.approx(0.93)

    def test_empty_dict(self):
        assert _flatten_calib_stats({}) == {}

    def test_no_unprefixed_keys(self):
        flat = _flatten_calib_stats(CALIB_STATS)
        assert all(k.startswith("calib_") for k in flat)


# ---------------------------------------------------------------------------
# summarize_grid — calib stats ingestion
# ---------------------------------------------------------------------------

def _make_calib_stats(subdir, rmse=0.08, filename="tfs_calib_stats.json"):
    data = dict(CALIB_STATS)
    data["rmse"] = rmse
    with open(os.path.join(str(subdir), filename), "w") as fh:
        json.dump(data, fh)
    return data


class TestSummarizeGridCalibStats:

    def test_calib_columns_present(self, tmp_path):
        sub = tmp_path / "run_0001"
        sub.mkdir()
        _make_combo(str(sub))
        _make_calib_stats(sub)
        df = summarize_grid(str(tmp_path), out_prefix=str(tmp_path / "out"))
        assert "calib_rmse" in df.columns
        assert "calib_n_params" in df.columns
        assert "calib_pearson_r" in df.columns

    def test_calib_values_correct(self, tmp_path):
        sub = tmp_path / "run_0001"
        sub.mkdir()
        _make_combo(str(sub))
        _make_calib_stats(sub, rmse=0.11)
        df = summarize_grid(str(tmp_path), out_prefix=str(tmp_path / "out"))
        assert df.loc[0, "calib_rmse"] == pytest.approx(0.11)
        assert df.loc[0, "calib_n_params"] == 12

    def test_run_without_calib_stats_gets_nan(self, tmp_path):
        sub1 = tmp_path / "run_0001"
        sub2 = tmp_path / "run_0002"
        sub1.mkdir()
        sub2.mkdir()
        _make_combo(str(sub1))
        _make_calib_stats(sub1, rmse=0.06)
        _make_combo(str(sub2))
        df = summarize_grid(str(tmp_path), out_prefix=str(tmp_path / "out"))
        row1 = df[df["run"] == "run_0001"].iloc[0]
        row2 = df[df["run"] == "run_0002"].iloc[0]
        assert row1["calib_rmse"] == pytest.approx(0.06)
        assert pd.isna(row2["calib_rmse"])

    def test_calib_and_fit_summary_coexist(self, tmp_path):
        sub = tmp_path / "run_0001"
        sub.mkdir()
        _make_combo(str(sub))
        _make_fit_summary(str(sub), training_rmse=0.05)
        _make_calib_stats(sub, rmse=0.09)
        df = summarize_grid(str(tmp_path), out_prefix=str(tmp_path / "out"))
        assert df.loc[0, "theta_training_rmse"] == pytest.approx(0.05)
        assert df.loc[0, "calib_rmse"] == pytest.approx(0.09)

    def test_malformed_calib_json_skipped_gracefully(self, tmp_path):
        sub = tmp_path / "run_0001"
        sub.mkdir()
        _make_combo(str(sub))
        with open(os.path.join(str(sub), "tfs_calib_stats.json"), "w") as fh:
            fh.write("{ not valid json }")
        df = summarize_grid(str(tmp_path), out_prefix=str(tmp_path / "out"))
        assert len(df) == 1
        assert "calib_rmse" not in df.columns or pd.isna(df.loc[0, "calib_rmse"])
