...
def mock_data():
    ...
    map_condition_pre = torch.tensor([0, 2, 4, 1], dtype=torch.int32)
    map_condition_sel = torch.tensor([5, 3, 1, 0], dtype=torch.int32)
...
def test_get_hyperparameters(mock_data):
    ...
    assert torch.allclose(k_loc, torch.tensor(0.025))
...
def test_define_model_logic_and_shapes(mock_data):
    ...
    k_per_cond_rep_1d = model_trace.nodes[k_name]["value"]
    m_per_cond_rep_1d = model_trace.nodes[m_name]["value"]
    ...
    expected_k_vals = expected_k_dist_2d.reshape(-1)
    expected_m_vals = expected_m_dist_2d.reshape(-1)

    # Now, the comparison should be correct
    assert torch.allclose(k_per_cond_rep_1d, expected_k_vals)
    assert torch.allclose(m_per_cond_rep_1d, expected_m_vals)
...
def test_guide_logic_and_shapes(mock_data):
    ...
    k_hl_loc = guide_trace.nodes[f"{name}_k_hyper_loc_loc"]["value"]
    ...
    k_offset_locs = guide_trace.nodes[f"{name}_k_offset_locs"]["value"]
    ...
    k_hyper = guide_trace.nodes[f"{name}_k_hyper_loc"]["value"]
    ...
    k_offset = guide_trace.nodes[f"{name}_k_offset"]["value"]
