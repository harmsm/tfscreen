...
def test_get_guesses(mock_data):
    ...
    assert torch.allclose(guesses[f"{name}_offset"][0], torch.tensor(-0.8240460108562919))
...
def test_define_model_logic_and_shapes(mock_data):
    ...
    assert torch.all(dk_geno_site[wt_indices] == 0.0)
    ...
    expected_lognormal = torch.clamp(torch.exp(hyper_loc + offset_val * hyper_scale), max=1e30)
    expected_mutant_val = hyper_shift - expected_lognormal

    # Check that mutant values match the calculation
    assert torch.allclose(mutant_values, expected_mutant_val)

    # Because of the specific guess value, this should be close to 0.0
    assert torch.allclose(mutant_values, torch.tensor(0.0))
