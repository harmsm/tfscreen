...
def mock_growth_model(mocker):
    mock_gm_class = mocker.patch("tfscreen.analysis.hierarchical.growth_model.scripts.run_growth_analysis.read_configuration")
...
def test_analyze_theta_checkpoint_exists_with_resume(mock_growth_model, mock_run_inference):
    ...
    with patch("os.path.exists", return_value=True):
        with patch("tfscreen.analysis.hierarchical.growth_model.scripts.run_growth_analysis._run_svi") as mock_run_svi:
            run_growth_analysis(
                config_file="config.yaml",
                seed=1,
                out_root="test_root",
