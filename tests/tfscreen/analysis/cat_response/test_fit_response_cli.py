"""
Tests for fit_response_cli.py — theta_col auto-detection and error handling.
"""
import pytest
import pandas as pd
from unittest.mock import patch

from tfscreen.analysis.cat_response.fit_response_cli import fit_response
...
_SYNC_EXECUTOR = patch(
    "tfscreen.analysis.cat_response.fit_response_cli.ProcessPoolExecutor",
...
class TestFitResponseThetaColAutoDetect:
    ...
    def test_autodetects_point_est(self):
        ...
        with patch("tfscreen.analysis.cat_response.fit_response_cli.cat_fit",
                   side_effect=fake_fit), _SYNC_EXECUTOR:
            fit_response(_make_df("point_est"), models_to_run=["flat"])
    ...
    def test_explicit_theta_col_overrides(self):
        ...
        with patch("tfscreen.analysis.cat_response.fit_response_cli.cat_fit",
                   side_effect=fake_fit), _SYNC_EXECUTOR:
            fit_response(df, theta_col="my_col", models_to_run=["flat"])
